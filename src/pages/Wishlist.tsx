...
//         {/* Recommended Section */}
//         {!loading && !error && (allWishList?.data?.length || 0) === 0 && (
...
const Wishlist = () => {
  ...
  // Reset to page 1 when wishlist becomes empty
  useEffect(() => {
    if (allWishList?.data?.length === 0) {
      handlePageChange(1);
    }
  }, [allWishList?.data?.length, handlePageChange]);
